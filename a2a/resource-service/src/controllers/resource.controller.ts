...
export class ResourceController {
  ...
  /**
   * POST `/api/pay-voucher`
   *
   * Settles a streaming (channel-based) payment via the CronosStream Sequencer.
   *
   ...
   *
   * @param req - Express request containing voucher fields in `req.body`.
   * @param res - Express response.
   * @param next - Express next middleware function.
   */
  public async payVoucher(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
    try {
      const { paymentId, channelId, amount, receiver, sequenceNumber, timestamp, userSignature } = req.body ?? {};

      if (!paymentId || !channelId || !amount || !receiver || !userSignature) {
        return res.status(HttpCode.BadRequest).json({ error: 'missing voucher fields' });
      }
