...
/**
 * Records a voucher-based payment as entitled.
 *
 * @param paymentId - The payment id to mark as settled.
 */
export function handleVoucherPayment(paymentId: string): void {
  paid.set(paymentId, { settled: true, at: Date.now() });
}
...
export const requireX402Payment = (options: RequireX402Options) => {
  ...
  return (req: Request, res: Response, next: NextFunction): void => {
    ...
    // Streaming (channel-based) payment option for high-frequency payments
    const streamingAccepts: Accepts = {
      scheme: 'streaming',
      network,
      asset,
      payTo,
      maxAmountRequired,
      maxTimeoutSeconds,
      description: `${description} (via payment channel)`,
      mimeType,
      resource,
      outputSchema,
      extra: {
    ...
    const response: X402Response = {
      ...
      accepts: [accepts, streamingAccepts],
    };
