...
```mermaid
flowchart LR
    HTTP[HTTP Request] --> MW[x402 Middleware]
    MW --> CTRL[Controller]
    CTRL --> SVC[Resource Service]
    SVC --> FAC[X402 Facilitator]

    SVC -->|Entitlement OK| RESP[Protected Response]
```
...
**Streaming Scheme Support** — accepts **EIP-712** off-chain vouchers for high-frequency access.
**Sequencer Integration** — validates vouchers via the external CronosStream Sequencer.
