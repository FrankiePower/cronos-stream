...
class PaywallService:
    ...
    async def fetch_resource(self, target: dict) -> dict:
        ...
        async with httpx.AsyncClient(timeout=20.0) as client:
            ...
            if not accepts0:
                 # Fallback to first if neither matches explicitly (compatibility)
    ...
    async def handle_streaming_payment(self, client: httpx.AsyncClient, resource_url: str, base_url: str, accepts: dict) -> dict:
        """
        Handles the streaming payment flow using CronosStream Sequencer.
        """
        ...
        # Add required paymentId for settlement correlation
        voucher["paymentId"] = payment_id
        
        # POST to /api/pay-voucher
        # We assume the resource service follows convention or we reconstruct from base
        pay_endpoint = f"{base_url}/api/pay-voucher"
        
        print(f"Submitting voucher to {pay_endpoint}...")
        res = await client.post(pay_endpoint, json=voucher)
        
        if res.status_code >= 400:
            raise NetworkError(f"Voucher settlement failed: {res.text}")
            
        print("Voucher settled! Retrying resource...")
        # Retry request
        retry = await client.get(
            resource_url, headers={X_PAYMENT_ID_HEADER: payment_id}
        )
        ...
        if retry.status_code != 200:
             raise NetworkError(f"Retry failed: {retry.status_code} {retry.text}")
