...
3. The agent parses the challenge and generates either an **EIP-3009 authorization** (Exact) or **EIP-712 Voucher** (Streaming).
4. The agent submits the signed proof to the backend.
5. The backend settles the payment (via Facilitator or Sequencer).
...
flowchart LR
  ...
  subgraph Monorepo["x402 A2A Agent ↔ Resource Monorepo"]
    RS["resource-service/<br/>(Node.js backend)<br/>• x402-protected endpoints<br/>• /api/pay-voucher"]
    AG["a2a-agent/<br/>(Python A2A agent)<br/>• AgentCard discovery<br/>• EIP-712 Signing<br/>• Streaming Payments"]
  end
  
  SEQ["Sequencer Service<br/>(Rust / External)"]
...
- **x402 protocol**
- **EIP-712 (Streaming Scheme)**
- **CronosStream Sequencer (Rust)**
